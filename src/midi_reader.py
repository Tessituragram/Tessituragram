...
class MidiParser:
    ...
    def parse_controller_event(self, channel_number, v_time):
        ...
        return [
            events.ControllerEvent(time, channel_number, controller_number, value)
    ...
    def parse_program_change_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.ProgramChangeEvent(time, channel_number, program_number)
    ...
    def parse_note_on_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.NoteOnEvent(time, channel_number, note_number, velocity)
    ...
    def parse_note_off_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.NoteOffEvent(time, channel_number, note_number, velocity)
            for time, note_number, velocity in zip(times, note_numbers, velocities)
        ]
    ...
    def parse_channel_aftertouch_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.ChannelAftertouchEvent(time, channel_number, amount)
    ...
    def parse_note_aftertouch_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.NoteAftertouchEvent(time, channel_number, note_number, amount)
    ...
    def parse_smpte_offset(self, v_time):
        ...
        self.byte_counter += 5
        return events.MetaEvent(
            v_time,
            "SMPTE Offset",
            utils.SMPTEOffset(hours, minutes, seconds, frames, subframes),
        )
    ...
    def parse_pitch_bend_event(self, channel_number, v_time):
        ...
        self.byte_counter -= length + 1
        return [
            events.PitchBendEvent(time, channel_number, lsb, msb)
    ...
    def parse_midi_event(self) -> events.MidiEvent:
        ...
        if event < 0x80:
            ...
            print(
                "Could not read MIDI file. Found an event with invalid status "
                + str(event)
                + "."
            )
            return -1
            ...
            match meta_type:
                ...
                case 0x02:
                    return self.parse_meta(
                        v_time, "Copyright Notice", v_length, decode=True
                    )
                case 0x03:
                    return self.parse_meta(
                        v_time, "Sequence or Track Name", v_length, decode=True
                    )
                case 0x04:
                    return self.parse_meta(
                        v_time, "Instrument Name", v_length, decode=True
                    )
                case 0x05:
                ...
                case 0x20:
                    return self.parse_meta(
                        v_time, "MIDI Channel Prefix Assignment", v_length
                    )
                case 0x21:
                ...
                case _:
                    ...
                    print(
                        f"Skipped unknown meta event 0x{meta_type:02X} of length {v_length}"
                    )
                    return self.parse_meta(
                        v_time, f"Unknown Meta Event 0x{meta_type:02X}", v_length
                    )
    ...
    def parse_header(self):
        ...
        if is_midi != bytes("MThd", "utf-8"):
            self.error_message = (
                "This doesn't appear to be a valid MIDI file -- MIDI header invalid."
            )
        ...
        if header_size != 6:
            self.error_message = (
                "This doesn't appear to be a valid MIDI file (unexpected header size)."
            )
            print("Invalid file type.")
            return -1
        ...
        if self.number_tracks != 1 and self.number_tracks != 2:
            ...
            print(
                "Incorrect number of tracks. There should only be 1 or 2 tracks. Found "
                + str(self.number_tracks)
                + " tracks."
            )
            return -1
    ...
    def parse_midi(self):
        ...
        if self.number_tracks == 2:
            ...
            meta_track = None
            if self.contains_notes(events_track_1) and self.contains_notes(
                events_track_2
            ):
    ...
    def get_notes(self):
        ...
        for event in self.events:
            ...
            if isinstance(event, events.NoteOnEvent) and event.velocity > 0:
                if note_number:
                    self.error_message = (
                        "This MIDI file contains overlapping notes (a new note starts before the previous one ends). "
    ...
    def post_process(self, notes):
        ...
        if (
            notes
            and isinstance(notes[-1], events.Note)
            and round(notes[-1].frequency, 2) == 32.70
        ):
            notes.pop()
        else:
