...
class Tessitura:
    ...
    def calculate_tessitura(self, notes):
        ...
        self.lowFreq = round(
            utils.weighted_percentile_expand(pitches, 25, weights=durations), 1
        )
        self.highFreq = round(
            utils.weighted_percentile_expand(pitches, 75, weights=durations), 1
        )
        ...
        self.median_freq = round(
            utils.weighted_percentile_expand(pitches, 50, weights=durations), 1
        )
        self.median_freqNote, self.median_freqOctave = utils.freq_to_note(
            self.median_freq
        )
...
class TessPassContainer:
    ...
    def print_tessitura(self):
        ...
        print(
            "Compositional Range: "
            + str(self.tess.min_pitch)
            + "\u2013"
            + str(self.tess.max_pitch)
            + " Hz,",
            self.tess.min_pitch_note
            + utils.convert_digit_to_subscript(self.tess.min_pitch_octave)
            + "\u2013"
            + self.tess.max_pitch_note
            + utils.convert_digit_to_subscript(self.tess.max_pitch_octave),
        )
        print(
            "Tessitura Range: ",
            str(self.tess.lowFreq) + "\u2013" + str(self.tess.highFreq) + " Hz,",
            self.tess.lowNote
            + utils.convert_digit_to_subscript(self.tess.lowOctave)
            + "\u2013"
            + self.tess.highNote
            + utils.convert_digit_to_subscript(self.tess.highOctave),
        )
        print(
            "Median Frequency: " + str(self.tess.median_freq) + " Hz,",
            "~"
            + self.tess.median_freqNote
    ...
    def print_passagio(self):
        print(
            "High Voice High Passaggio: ",
            str(self.passa.hvhp.time_dose) + "s,",
            str(self.passa.hvhp.percentage) + "%",
        )

        print(
            "High Voice Middle Passaggio: ",
            str(self.passa.hvmp.time_dose) + "s,",
            str(self.passa.hvmp.percentage) + "%",
        )

        print(
            "High Voice Low Passaggio: ",
            str(self.passa.hvlp.time_dose) + "s,",
            str(self.passa.hvlp.percentage) + "%",
        )

        print(
            "Medium Voice High Passaggio: ",
            str(self.passa.mvhp.time_dose) + "s,",
            str(self.passa.mvhp.percentage) + "%",
        )

        print(
            "Medium Voice Middle Passaggio: ",
            str(self.passa.mvmp.time_dose) + "s,",
            str(self.passa.mvmp.percentage) + "%",
        )

        print(
            "Medium Voice Low Passaggio: ",
            str(self.passa.mvlp.time_dose) + "s,",
            str(self.passa.mvlp.percentage) + "%",
        )

        print(
            "Low Voice High Passaggio: ",
            str(self.passa.lvhp.time_dose) + "s,",
            str(self.passa.lvhp.percentage) + "%",
        )

        print(
            "Low Voice Middle Passaggio: ",
            str(self.passa.lvmp.time_dose) + "s,",
            str(self.passa.lvmp.percentage) + "%",
        )

        print(
            "Low Voice Low Passaggio: ",
            str(self.passa.lvlp.time_dose) + "s,",
            str(self.passa.lvlp.percentage) + "%",
        )
        print("=================================================")
    ...
    def write_to_pdf(self):
        ...
        self.pdf.cell(
            0, 6, f'"{self.filename}"', new_x="LMARGIN", new_y="NEXT", align="C"
        )
        self.pdf.cell(
            0, 6, "Musical Demand Profile", new_x="LMARGIN", new_y="NEXT", align="C"
        )
        ...
        self.pdf.set_font("times_new", "", 12)
        self.pdf.write_html(
            f"Cycle Dose (<i>\u0192<sub>p</sub>t<sub>p</sub></i>): {self.tess.cycle_dose} vibrations"
        )
        self.pdf.ln(1)
        self.pdf.set_x(40)
        self.pdf.write_html(
            f"Total Time (<i>t</i>): {convert_to_minutes(self.tess.total_time)} minutes, {self.tess.total_time} s"
        )
        self.pdf.ln(1)
        self.pdf.set_x(40)
        self.pdf.write_html(
            f"Time Dose (<i>t<sub>p</sub></i>): {convert_to_minutes(self.tess.time_dose)} minutes, {self.tess.time_dose} s"
        )
        self.pdf.ln(1)
        self.pdf.set_x(40)
        self.pdf.write_html(
            f"Rest Time (<i>t<sub>r</sub></i>): {convert_to_minutes(self.tess.rest_time)} minutes, {self.tess.rest_time} s"
        )
        ...
        self.pdf.set_font("times_new", "", 12)
        self.pdf.write_html(
            f"Range (<i>Q<sub>0p</sub>–Q<sub>4p</sub></i>): {self.tess.min_pitch_note}<sub>{str(self.tess.min_pitch_octave)}</sub><i>-</i>{self.tess.max_pitch_note}<sub>{str(self.tess.max_pitch_octave)}</sub>, {self.tess.min_pitch}–{self.tess.max_pitch} Hz"
        )
        self.pdf.ln(1)
        self.pdf.set_x(40)
        self.pdf.write_html(
            f"Tessitura (<i>Q<sub>1p</sub>–Q<sub>3p</sub></i>): ≈{self.tess.lowNote}<sub>{str(self.tess.lowOctave)}</sub>-≈{self.tess.highNote}<sub>{str(self.tess.highOctave)}</sub>, {self.tess.lowFreq}–{self.tess.highFreq} Hz"
        )
        self.pdf.ln(1)
        self.pdf.set_x(40)
        self.pdf.write_html(
            f"Median \u0192<sub>o</sub> (<i>Q<sub>2p</sub></i>): ≈{self.tess.median_freqNote}<sub>{str(self.tess.median_freqOctave)}</sub>, {self.tess.median_freq} Hz"
        )
        self.pdf.ln(3)
        ...
        if tess_success == 1:
            tess_fp = os.path.join(
                self.output_dir,
                self.filename + "-Tessitura-" + self.passa.clef_range + "-1.png",
            )
            ...
            self.pdf.cell(
                0,
                5,
                "NO TESSITURA / RANGE IMAGE: MUSESCORE FILEPATH NOT FOUND.",
                align="C",
                fill=True,
            )
            self.pdf.ln(5)
        ...
        hv_arr = [
            self.passa.hvhp.percentage,
            self.passa.hvmp.percentage,
            self.passa.hvlp.percentage,
        ]
        self.pdf.cell(35, 5, f"{round(sum(hv_arr), 1)}%", border=0, align="C")
        mv_arr = [
            self.passa.mvhp.percentage,
            self.passa.mvmp.percentage,
            self.passa.mvlp.percentage,
        ]
        self.pdf.cell(35, 5, f"{round(sum(mv_arr), 1)}%", border=0, align="C")
        lv_arr = [
            self.passa.lvhp.percentage,
            self.passa.lvmp.percentage,
            self.passa.lvlp.percentage,
        ]
        ...
        images = [
            (passaggio_images["High Voice"], "HV"),
            (passaggio_images["Medium Voice"], "MV"),
            (passaggio_images["Low Voice"], "LV"),
        ]
        ...
        if os.path.exists(
            os.path.join(
                self.output_dir,
                self.filename + "-Tessitura-" + self.passa.clef_range + ".musicxml",
            )
        ):
            os.remove(
                os.path.join(
    ...
    def generate_tess_image(self):
        ...
        low = convert_to_music_21_note(
            self.tess.min_pitch_note, self.tess.min_pitch_octave
        )
        ...
        high = convert_to_music_21_note(
            self.tess.max_pitch_note, self.tess.max_pitch_octave
        )
        ...
        fp = os.path.join(
            self.output_dir, self.filename + "-Tessitura-" + self.tess.clef_range
        )
        try:
    ...
    def _render_voice(self, voice_type_pass, voice_type):
        ...
        font = ImageFont.truetype(
            utils.resource_path(os.path.join("data", "Times New Roman.ttf")), size=48
        )
        ...
        draw.text(
            (550, voice_type_pass[0][1]),
            str(voice_type_pass[0][0]) + "%",
            fill="black",
            font=font,
        )
        draw.text(
            (550, voice_type_pass[1][1]),
            str(voice_type_pass[1][0]) + "%",
            fill="black",
            font=font,
        )
        draw.text(
            (550, voice_type_pass[2][1]),
            str(voice_type_pass[2][0]) + "%",
            fill="black",
            font=font,
        )
    ...
    def generate_passaggio_image(self):
        hv_notes = [
            (self.passa.hvhp.percentage, 50),
            (self.passa.hvmp.percentage, 195),
            (self.passa.hvlp.percentage, 260),
        ]
        mv_notes = [
            (self.passa.mvhp.percentage, 70),
            (self.passa.mvmp.percentage, 217),
            (self.passa.mvlp.percentage, 285),
        ]
        lv_notes = [
            (self.passa.lvhp.percentage, 90),
            (self.passa.lvmp.percentage, 237),
            (self.passa.lvlp.percentage, 305),
        ]

        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = {
                "High Voice": executor.submit(
                    self._render_voice, hv_notes, "High Voice"
                ),
                "Medium Voice": executor.submit(
                    self._render_voice, mv_notes, "Medium Voice"
                ),
